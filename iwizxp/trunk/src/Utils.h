...
namespace Utils {
	 ...
	 * @throws RegexException When the regex could not be compiled or
	 *         executed.
	 */
	 ...
	 *
	 * @return A vector with the names of all the available network
	 *         interfaces.
	 */ 
	...
	namespace DOM {
		...
		/**
		 * This method reads the entire given stream, and attempts to
		 ...
		 * 
		 * @param inStream The input stream to read the XML document
		 *        from.
		 * @return The DOM representation of the documents that was in
		 *         the stream.
		 * @throws DOMParseException Thrown when the parser isn't
		 *         available from the global repository, or when the
		 *         parser could not parse the given stream.
		 */
		xercesc::DOMDocument *parseDocumentFromStream(
			std::istream &inStream);

		/**
		 * Recursively goes over the tree that <code>node</code> is its
		 * root, and removes any text node that cotains whitespaces
		 * alone (should be done by the normalization, but doesn't for
		 * some reason).
		 *
		 * TODO: If someone could tell me how to make Xerces' DOMBuilder
		 * to act as it should, please tell me. This method is not a
		 * method I like...
		 *
		 * @param node Any node that's under this node will be removed,
		 *        if its an ignoreable text node.
		 */
		 ...
		 * @return <code>true</code> if the given node is a text node
		 *         that holds nothing but whitespaces.
		 */
		bool isIgnoreable(xercesc::DOMNode *node);

		/**
		 * This method traverses the given tree up to a certain depth,
		 * and looks for elements with the given name.
		 *
		 * @param result Result vector to store the elements found. Note
		 *        that the vector will not be emptied upon calling this
		 *        method. Elements are appended at the end of the vector
		...
		/**
		 * This method looks for a single elements that matches the
		 * given name up to the given depth from the root. In case it
		 * find more than on mtching element, a warning message is
		 * issued, and the first element found is returned.
		 *
		 ...
		 * @return The first element that's found (in case we found more
		 *         than one, or <code>null</code>, in case nothing was
		 *         found.
		 */
		 ...
		 * @return Attribute's value (or an empty string if the
		...
		/**
		 * This method builds a sorted array of <code>DOMElement</code>s
		 * from the special structure defined in the XML for an array.
		 *
		 * Example:
		 * The XML document:
		 * <pre>
		 ...
		 * &lt;document&gt;
		 * &nbsp;&lt;array count="3"&gt;
		 * &nbsp;&nbsp;&lt;item item="1"&gt;value1&lt;/item&gt;
		 * &nbsp;&nbsp;&lt;item item="3"&gt;value3&lt;/item&gt;
		 * &nbsp;&nbsp;&lt;foo&gt;bar&lt;/foo&gt;
		 * &nbsp;&nbsp;&lt;item item="2"&gt;value2&lt;/item&gt;
		 * &nbsp;&lt;/array&gt;
		 * &lt;/document&gt;
		 * </pre>
		 ...
		 * </pre>
		 * Even though the elements are not sorted correctly in the
		 * document and there were unrelated elements in the middle of
		 * the list (they are simply ignored).
		 *
		 * <b>Note</b>: The count in the XML is one-based, while
		 * zero-based in the result vector.
		 *
		 * In case of format failure in the array element,
		 * <code>result</code> will hold an empty vector of unknown
		 * length.
		 *
		 * In case of format failure in one of the elements, its entry
		 * in the vector will hold <code>null</code>.
		 *
		 * @param result Vector to store the results in. <b>Note</b>:
		 *        The given <code>vector</code> will be emptied and
		 *        resized - Data in the vector will be lost!
		 * @param arrayNode The node that begin the array (someArray's
		 *        element in the example). This elements must have the
		 *        attribute <code>countAttribute</code> that holds a
		 *        valid decimal number representation that could be
		 *        parsed using <code>atoi()</code> to an integer. Value
		 *        of this attribute must match exactly the number of
		 *        <code>itemTag</code>s under <code>arrayNode</code>.
		 * @param itemTag The name of the tag of the elements that's
		 *        under <code>arrayNode</code> that are list elements
		 ...
		 * @param itemAttribute The name of the attribute found under
		 *        each <code>itemTag</code> that holds the
		 *        representation of the index of the elements in the
		 *        array. This value should be <i>one-based</i>
		 *        (<code>item</code> by default, and in the example).
		 */
