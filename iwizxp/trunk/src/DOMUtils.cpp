...
DOMDocument *Utils::DOM::parseDocumentFromStream(istream &inStream) {
	...
	try {
		DOMBuilder *builder =
			GlobalRepository::getInstance()->getDOMBuilder();
		
		if (builder == 0) {
			throw DOMParseException("Unable to get DOMBuilder "
...
DOMElement *Utils::DOM::getLoneElementByTagName(DOMElement *root,
	...
	if (matchingNodes.size() > 1) {
		Log::warning(LOG_LOCATION("Utils::DOM",
			"getLoneElementByTagName"),
				string("More than one <" +
				tagName + "> tag found, using first."));
	}
	...
	if (root->hasChildNodes() && (level < depth)) {
		...
		if (child->getNodeType() == DOMNode::ELEMENT_NODE) {
			getElementsByTagName(result,
				dynamic_cast<DOMElement*>(child), tagName,
				depth, level + 1);
		}
	}
...
void Utils::DOM::elementsArrayFromXML(
	...
	// TODO: This method is too ling. refactor-extract method.
	// Empty vector
	result.empty();

	// How much elements?
	string countAttributeValue =
		getAttributeValue(arrayNode, countAttribute);
	...
	while (child != 0) {
		if (xts(child->getNodeName()).asString() != itemTag) {
			...
			string itemNumber =
				getAttributeValue(child, itemAttribute);

			// Check attribute
			if (itemNumber.length() == 0) {
				// No item attribute, skip
				Log::debug(LOG_LOCATION("Utils::DOM",
					"elementsArrayFromXML"),
					"No '" + itemAttribute +
					"' attribute, ignoring.");
			} else {
