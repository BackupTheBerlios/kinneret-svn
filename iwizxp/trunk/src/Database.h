...
private:
	 ...
	 *
	 * The constructor attempt to load every available XML file that's in
	 * the appropriate directory. If a file is of incorrect format, it is
	...
	/**
	 * Destructor. Releases all the <code>Isp</code>s and
	 * <code>Modem</code>s that we allocated during construction.
	 */
	virtual ~Database();
...
public:
	...
	/**
	 * This method releases the database. Call it once you're done. It
	 * should start the chain of releases of the stuff we loaded in the
	 * constructor.
	 */
	static void release() {
		delete instance;
	...
	/**
	 * The default pre dialer is a regular dialer, with the exception that
	 * it doesn't dial anywhere. It is a meta-dialer, common to all the
	 * dialers we're about to create with this wizard, which defines some
	 * scripts that should be ran before and after the actual connection or
	 * disconnection code.
	 *
	 * The purpose of this dialer is to prepare the enviroment for the
	 * connection code, usually setting some variables the scripts relies
	 * on, and setting system data like ppp sercers.
	 *
...
private:
	...
	/**
	 * This private method attempts to load every file that
	 * <code>enumIsps()</code> enumerated using
	 * <code>loadIspFromFile()</code>.  In case a load failes (due to
	 * incorrect format, invalid path, whatever) the method skips the isp
	 * and continues on. In case there were no successful loads, the method
	 * notifies the caller with a <code>DatabaseCreationException</code>.
	 *
	 * @throws DatabaseCreationException Thrown when the loader wasn't able
	...
	/**
	 * The method releases every ISP allocated using
	 * <code>loadIsps()</code>.
	 */
	void releaseIsps();
